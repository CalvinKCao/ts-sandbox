#!/usr/bin/env bash
# Narval A100 launcher for the gated oracle-coarse smoke test.
# Login node: ./submit_ordinal_patch_refinement_narval_a100.sh --steps 200
set -euo pipefail
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
    ...
    exec sbatch --job-name=ordinal-refine-smoke --account=def-boyuwang \
        ...
        "$SCRIPT_DIR/submit_ordinal_patch_refinement_narval_a100.sh"
fi
...
OUTPUT="$REPO/results/ordinal_patch_refinement_killtest/narval-smoke-${SLURM_JOB_ID}"
python -u -m experiments.ordinal_patch_refinement_killtest.run_narval_smoke \
