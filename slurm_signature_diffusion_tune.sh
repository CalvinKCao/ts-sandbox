...
#   WORKERS=12 MAX_CONCURRENT=12 bash slurm_signature_diffusion_tune.sh
#   SMOKE_TEST=1 bash slurm_signature_diffusion_tune.sh
#   BUILD_SHARED_VENV=1 bash slurm_signature_diffusion_tune.sh   # once on login node (avoids CVMFS pip flakes)
#
# Finalize (full test eval on best trial) is auto-submitted with
...
if [ ! -f "$SCRIPT_DIR/slurm_signature_diffusion_tune.sh" ]; then
...
if [ -z "${SLURM_JOB_ID:-}" ]; then
    if [ "${BUILD_SHARED_VENV:-0}" = "1" ]; then
        ...
        if [ -z "${STORE:-}" ] && [ -d "$HOME/projects" ]; then
            ...
            shopt -u nullglob
            [ "${#matches[@]}" -gt 0 ] && STORE="$(readlink -f "${matches[0]}")/$USER/ts-sandbox-sigdiff"
        fi
    ...
    MANIFEST="$SCRIPT_DIR/results/signature_diffusion/last_submission.json"
    python - <<PY
import json
import os
...
manifest = {
    ...
    "study_name_pattern": "signature_diffusion_{dataset}_job${ARRAY_JOB_ID}",
...
echo "=========================================="
echo "Signature diffusion Optuna worker"
...
if [ ! -f "$VENV_HELPER" ]; then
    ...
    echo "Submit from the repo checkout, e.g. cd \$SCRATCH/ts-sandbox && bash slurm_signature_diffusion_tune.sh"
    exit 1
fi
# shellcheck source=cluster/setup_signature_cluster_venv.sh
source "$VENV_HELPER"
...
STORE="${STORE:-$STORE_BASE/$USER/ts-sandbox-sigdiff}"
mkdir -p "$STORE"
...
if [ "${RESUME_STUDY:-0}" = "1" ]; then
    STUDY_NAME="${RESUME_STUDY_NAME:-signature_diffusion_${DATASET}_v1}"
else
    STUDY_NAME="${STUDY_NAME:-signature_diffusion_${DATASET}_job${SLURM_ARRAY_JOB_ID:-local}}"
fi
