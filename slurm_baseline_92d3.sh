#!/bin/bash
# =============================================================================
# Gaussian DiT baseline diffusion (standard epsilon-prediction, no anchor loss) for 92d3.
#
# USAGE (from repo root on Killarney login node, preferably $SCRATCH/ts-sandbox):
#   ./slurm_baseline_92d3.sh --dataset ETTh1
#   ./slurm_baseline_92d3.sh --dataset exchange_rate
#   ./slurm_baseline_92d3.sh --dataset ETTh2
#   ./slurm_baseline_92d3.sh --dataset ETTh1 --smoke-test
#   ./slurm_baseline_92d3.sh --dataset ETTh1 --resume
# =============================================================================
...
WALL="${WALL:-}"
EVAL_SAMPLER="dpmpp"
WANDB_PROJECT="${WANDB_PROJECT:-ts-sandbox-baseline-92d3}"
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
    if [[ "$SMOKE" -eq 1 ]]; then
        ...
        MEM="24G"
        CPUS=4
        JOB_NAME="baseline-smoke"
    else
        [[ -z "$WALL" ]] && WALL="1-00:00:00"
        MEM="60G"
        CPUS=8
        JOB_NAME="baseline-${DATASET,,}"
    fi
    ...
    sbatch \
        ...
        "$SCRIPT_DIR/slurm_baseline_92d3.sh" \
...
resolve_92d3_run_dirs ckpts_flat "baseline-${DATASET,,}" "${DATASET,,}" 4
...
echo "[train] Gaussian DiT baseline training/eval..."
...
run = wandb.init(
    project=os.environ.get("WANDB_PROJECT", "ts-sandbox-baseline-92d3"),
    name="${RUN_STEM}-logs",
