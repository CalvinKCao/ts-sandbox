...
def plot_detailed_sample(dataset_name, run_dir, device, n_samples=2, v_idx=0):
    ...
    print(f"\nVisualizing {run_folder} (Scenario: {scenario}, Variate: {v_idx})")

    # 2. Load Data
    # Get TARGET_DIM
    target_dim = 7
    if "weather" in dataset_name.lower(): target_dim = 21
    if "exchange" in dataset_name.lower(): target_dim = 8
    ...
    # Use larger lookback if needed, but train_experimental_pipeline uses 512
    lookback = 512
    _, _, test_ds, norm_stats = load_dataset(dataset_name, lookback=lookback, horizon=96)
    ...
    # Find the best.pt
    best_pt = None
    # Check if there's a subdirectory with the best.pt
    ...
    for s_idx in range(n_samples):
        ...
        with torch.no_grad():
            ...
            # Use same seq_len as model
            seq_sl = getattr(itrans_model, 'seq_len', 512)
        ...
        # Extract 2D maps for the requested variate
        # past_2d: (B, V, H, W)
        ...
        past_dn = (past[v_idx, -96:] * s_v + m_v).numpy()
        gt_dn = (gt[v_idx] * s_v + m_v).numpy()
        pred_dn = (pred[v_idx, -96:] * s_v + m_v).numpy()
        ...
        if pn2d is not None:
            ...
            res_1d = (pred[v_idx, -96:] - res['trend'][0, v_idx].cpu()).numpy()
            # gt_residual
        ...
        plt.tight_layout()
        out_name = f"detailed_{scenario}_{dataset_name}_v{v_idx}_s{s_idx}.png"
...
def main():
    ...
    parser.add_argument('--v-idx', type=int, default=0)
    parser.add_argument('--n-samples', type=int, default=1)
    args = parser.parse_args()

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    plot_detailed_sample(args.dataset, args.run_dir, device, n_samples=args.n_samples, v_idx=args.v_idx)
