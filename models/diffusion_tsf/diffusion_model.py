...
- Conditional U-Net
- DDIM diffusion (gaussian + binary)
- 2D to 1D decoding
"""
...
try:
    ...
    from .guidance import GuidanceModel, iTransformerGuidance
...
class DiffusionTSF(nn.Module):
    ...
    2. Convert to 2D CDF occupancy map
    3. Apply vertical Gaussian blur (gaussian diffusion path)
    4. Train U-Net to denoise future conditioned on past
    5. At inference: generate future via DDIM
    6. Decode 2D representation back to 1D

    Hybrid "Visual Guide" mode (use_guidance_channel=True):
    - iTransformerGuidance generates a coarse forecast
    - Coarse forecast is converted to a 2D "ghost image"
    - Ghost image is concatenated to the U-Net input
    - Requires a guidance_model to be provided — raises ValueError otherwise
    """
    ...
    def __init__(
        ...
        Args:
            ...
            guidance_model: iTransformerGuidance instance. Required if
                            config.use_guidance_channel is True — no fallback.
        """
        super().__init__()
        self.config = config
        ...
        if config.use_guidance_channel:
            self.guidance_model = guidance_model  # may be None until set_guidance_model() is called
        else:
        ...
        if config.use_guidance_channel:
            logger.info(f"  Guidance channel: enabled (iTransformer → 2D ghost image)")
    ...
    def _inject_coordinate_channel(self, x: torch.Tensor) -> torch.Tensor:
        ...
        coord_grid = self._get_coordinate_grid(batch_size, height, width, x.device, x.dtype)
    ...
    def _inject_time_channels(self, x: torch.Tensor) -> torch.Tensor:
        if not self.config.use_time_ramp and not self.config.use_time_sine:
            return x
        batch_size, _, height, width = x.shape
        ramp, sine = self._get_time_features(batch_size, height, width, x.device, x.dtype)
        channels_to_add = [x]
    ...
    def _get_cross_variate_context(
        ...
        prefers iTransformer forecast when available (has cross-variate structure
        baked in), falls back to normalized past.
        """
        if self.context_encoder is None:
            return None

        if guidance_forecast_norm is not None:
            src = guidance_forecast_norm
        elif past_norm.dim() == 3:
            src = past_norm
        else:
            src = past_norm.unsqueeze(1)

        return self.context_encoder(src)
    ...
    def _compute_emd_loss(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """Column-wise Wasserstein-1 distance via CDF trick."""
        # both pred and target are in [-1, 1] CDF scale
    ...
    def encode_to_2d(self, x: torch.Tensor, scale_for_diffusion: bool = True) -> torch.Tensor:
        """Encode 1D time series to blurred 2D CDF representation."""
        image = self.to_2d(x)
        blurred = self.blur(image)
        if scale_for_diffusion:
            # map [0,1] CDF to [-1,1] for diffusion
    ...
    def encode_to_2d_binary(self, x: torch.Tensor) -> torch.Tensor:
        ...
        binary diffusion provides proper gradients for hard boundaries
        so we skip the gaussian blur entirely.
        """
        return self.to_2d(x)
    ...
    def decode_from_2d(
        ...
        """Decode 2D CDF representation back to 1D time series."""
        ...
        if num_vars > 1:
            raise NotImplementedError(f"decoder_method='{decoder_method}' not supported for multivariate.")
    ...
    def generate(
        ...
        """Generate future predictions using DDIM."""
        if self.config.variate_factorized and self.config.num_variables > 1:
            if self.config.diffusion_type == "binary":
            ...
            return self._generate_factorized(
                past, num_ddim_steps=num_ddim_steps,
        ...
        def model_fn(x_future, t, cond, use_null_context=False, use_null_guidance=False):
            if self.config.unified_time_axis:
                ...
                if self.config.use_guidance_channel:
                    ...
                    canvas_full = self._inject_guidance_channel(canvas_full, guide)
                ctx = null_encoder_hidden_states if use_null_context else encoder_hidden_states
                return self.noise_predictor(canvas_full, t, cond, encoder_hidden_states=ctx)
    ...
    def _forward_factorized(self, past: torch.Tensor, future: torch.Tensor, t: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        ...
        at the bottleneck via cross-attention on V context tokens from VariateCrossEncoder.
        """
        B = past.shape[0]
    ...
    @torch.no_grad()
    def _generate_factorized(self, past: torch.Tensor, num_ddim_steps: int = 50, eta: float = 0.0,
                              cfg_scale: Optional[float] = None, verbose: bool = False,
                              decoder_method: str = "mean") -> Dict[str, torch.Tensor]:
        """inference: per-variate DDIM with cross-variate bottleneck context."""
        B = past.shape[0]
        V = self.config.num_variables
        H = self.config.image_height
        device = past.device
    ...
    # ====================================================================
    # Binary diffusion — per-variate, no gaussian blur
    # ====================================================================
    ...
    def _boundary_bce_loss(
        ...
        pixels within `binary_boundary_width` rows of the boundary get weight
        `binary_boundary_weight`; everything else gets `binary_background_weight`.
        ...
        dist = (row_idx - filled_count.unsqueeze(2)).abs()
    ...
    def _generate_binary_factorized(
        ...
        future_2d   = future_2d_flat.reshape(B, V, H, W_fut)
        future_norm = self.decode_from_2d(future_2d, from_diffusion=False, decoder_method=decoder_method)
        future      = self._denormalize(future_norm, stats)
