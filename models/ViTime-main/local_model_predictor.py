...
class InferenceInterface:
    ...
    def inference(self, x, prediction_length=None, sampleNumber=None, tempature=1, return_prob_map=False):
        """
        Run inference and optionally return the per-timestep probability map.

        Args:
            x (np.ndarray): Input sequence of shape (T,) or (T, C).
            prediction_length (int): Forecast horizon.
            sampleNumber (int, optional): If set, sample from the distribution instead of expectation.
            tempature (float): Softmax temperature.
            return_prob_map (bool): If True, also return the softmax map (time x h).

        Returns:
            np.ndarray or (np.ndarray, np.ndarray): By default returns the de-normalized
            prediction sequence. If return_prob_map=True, also returns the probability map
            before numeric decoding (shape: time x h).
        """
        ...
        with torch.no_grad():
            ...
            # Save probability map (softmax over value axis) for the first channel
            prob_map = y_pred[0, 0].detach().cpu().numpy()  # shape (time, h)
            ...
            if return_prob_map:
                return y_pred_original, prob_map
            else:
