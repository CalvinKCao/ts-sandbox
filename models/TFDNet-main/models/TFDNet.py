...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.output_seasonal = getattr(configs, 'output_seasonal', False)
    ...
    def forward(self, x_enc):
        ...
        if self.output_seasonal:
            pred_seasonal = self.linear1(out_seasonal)
            pred_seasonal = self.dropout(pred_seasonal)
            pred_seasonal = pred_seasonal.reshape(B, variation, self.pred_len)
            pred_seasonal = pred_seasonal.permute(0, 2, 1)
            # pred_seasonal = self.revin_layer(pred_seasonal, 'denorm')
            # Fix: Only scale by stdev, do not add mean, as seasonal component should be zero-centered
            pred_seasonal = pred_seasonal * self.revin_layer.stdev
            return out, pred_seasonal
