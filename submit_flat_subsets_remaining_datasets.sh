...
# binary_anchor_stationary_flat_subsets_ema099 per dataset. MMPD MaskAE uses
# --subset-config (YAML) and does not wait on binary lr_lo ckpts.
#
...
LR_LO_CFG="configs/binary_anchor_stationary_flat_subsets_grad_accum_150_lr_lo.yaml"
...
if [[ "$SKIP_LR_LO" -eq 0 ]]; then
    echo "=== [2b/3] binary grad_accum_150_lr_lo ==="
    ./submit_grid.sh \
        --configs "$LR_LO_CFG" \
        --datasets "$DATASETS_CSV" \
        --time "$WALL_BINARY" \
...
if [[ "$SKIP_MASKAE" -eq 0 ]]; then
  echo "=== [3/3] MMPD MaskAE (subset YAML; no binary ckpt dependency) ==="
  MASKAE_DEP_ARGS=()
  if [[ "$SKIP_EMA099" -eq 0 && -n "$EMA_DEP" ]]; then
      MASKAE_DEP_ARGS=(--dependency "$EMA_DEP")
      echo "MaskAE after ema099: $EMA_DEP"
  fi
