#!/bin/bash
# =============================================================================
# Killarney L40S sweep for low-frequency iTransformer guidance + high-frequency
# residual diffusion with decoded 1D MSE + Soft-DTW.
#
# USAGE (from repo root on the Killarney login node):
#   bash slurm_soft_dtw_highfreq_sweep.sh
#   bash slurm_soft_dtw_highfreq_sweep.sh --smoke-test
#   bash slurm_soft_dtw_highfreq_sweep.sh ETTh1 weather
# =============================================================================
...
if [ -z "${SLURM_JOB_ID:-}" ]; then
    ...
    for ds in "${DATASETS[@]}"; do
        ...
        sbatch \
            ...
            --export="ALL,DATASET=$ds,SMOKE=$IS_SMOKE,RESIDUAL_CUTOFF_FREQ=${RESIDUAL_CUTOFF_FREQ:-0.12}" \
            "$SCRIPT_DIR/slurm_soft_dtw_highfreq_sweep.sh"
    done
    echo "All high-frequency Soft-DTW sweep jobs submitted."
    exit 0
fi
...
echo "Cutoff:   ${RESIDUAL_CUTOFF_FREQ:-0.12}"
...
export WANDB_PROJECT="${WANDB_PROJECT:-diffusion-tsf-softdtw-highfreq}"
...
COMMON_ARGS=(
    ...
    --model-type dit
    --experiment A
    --itrans-target lowpass
    --residual-cutoff-freq "${RESIDUAL_CUTOFF_FREQ:-0.12}"
    --subset-id "softdtw-highfreq-dit"
...
echo "Running Phase 1 (pretrain, low-pass iTransformer target + residual diffusion)..."
python -u -m models.diffusion_tsf.train_multivariate_pipeline \
    --mode pretrain \
    "${COMMON_ARGS[@]}"

echo "Running Phase 2 (finetune/eval, high-frequency residual diffusion)..."
python -u -m models.diffusion_tsf.train_multivariate_pipeline \
    --mode finetune \
    "${COMMON_ARGS[@]}"
...
if not run_id or not os.path.isfile(log_path):
    raise SystemExit(0)
run = wandb.init(project=os.environ.get("WANDB_PROJECT", "diffusion-tsf-softdtw-highfreq"),
