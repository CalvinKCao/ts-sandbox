#!/bin/bash
# Fourier conditioning + flatline-blur fine target on Narval (Killarney fallback).
#
# USAGE (Narval login node, from $SCRATCH/ts-sandbox):
#   ./submit_fourier_flatline_blur_narval.sh --smoke-test
#   ./submit_fourier_flatline_blur_narval.sh
#   ./submit_fourier_flatline_blur_narval.sh --datasets ETTh1,exchange_rate,weather
#
set -euo pipefail
