...
1. [C99](https://en.wikipedia.org/wiki/C99): Adds language and Standard Library features. <tt>int</tt> is no longer implicitly assumed.
1. [C11](https://en.wikipedia.org/wiki/C11_(C_standard_revision)): Major release adding memory model and concurrency (e.g., threads, atomics, compare-and-swap) support.
...
The reader should note that <tt>volatile</tt> in C and C++ has a very specific meaning, e.g., see [this page](http://en.cppreference.com/w/cpp/language/cv). Actually needing to use <tt>volatile</tt> in C/C++ code is a rare event and it is typically limited to certain kinds of low-level code.
