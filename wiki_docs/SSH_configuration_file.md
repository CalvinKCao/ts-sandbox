...
```bash
prompt=[name@yourLaptop ~]|ssh narval
```
This also changes the behaviour of <code>sftp</code>, <code>scp</code>, and <code>rsync</code> and you can now [transfer files](Transferring_data.md) by typing for example
```bash
prompt=[name@yourLaptop ~]|scp local_file narval:work/
```
If you frequently log into different clusters, modify the above <code>Host</code> block as follows instead of adding individual entries for each cluster separately:
  Host narval beluga graham cedar
    ...
    [...]
Note that you need to install your public [SSH key](SSH_Keys.md) on each cluster separately or use [CCDB](SSH_Keys#Using_CCDB.md).
