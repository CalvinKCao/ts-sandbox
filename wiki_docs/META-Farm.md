...
In META mode, the number of actual jobs (called <i>metajobs</i>) submitted by the package is usually much smaller than the number of cases to process. Each metajob can process multiple lines (multiple cases) from table.dat. A collection of metajobs will read lines from table.dat, starting from the first line, in a serialized manner using the [lockfile](https://linux.die.net/man/1/lockfile) mechanism to prevent a race condition. This ensures a good dynamic workload balance between metajobs, as metajobs which handle shorter cases will process more of them.
...
The META package has important advantages over other approaches like [GLOST](GLOST.md) where farm processing is done by bundling up all the jobs into a large parallel (MPI) job:
...
** [single_case.sh](#single_case.sh.md)
** [Example: Numbered input files](META:_Advanced_features_and_troubleshooting#Example:_Numbered_input_files.md) (advanced)
** [Example: Input file must have the same name](META:_Advanced_features_and_troubleshooting#Example:_Input_file_must_have_the_same_name.md) (advanced)
** [Using all the columns in the cases table explicitly](META:_Advanced_features_and_troubleshooting#Using_all_the_columns_in_the_cases_table_explicitly.md) (advanced)
...
** [single_case.sh](#single_case.sh.md)
** [STATUS and handling errors](#STATUS_and_handling_errors.md)
** [Example: Input file must have the same name](META:_Advanced_features_and_troubleshooting#Example:_Input_file_must_have_the_same_name.md) (advanced)
** [Using all the columns in the cases table explicitly](META:_Advanced_features_and_troubleshooting#Using_all_the_columns_in_the_cases_table_explicitly.md) (advanced)
- Modify the <code>job_script.sh</code> file to suit your needs as described at [job_script.sh](#job_script.sh.md) below. In particular, use a correct account name, and set an appropriate job runtime. For more about runtimes, see [Estimating the runtime and number of metajobs](#Estimating_the_runtime_and_number_of_metajobs.md).
...
=List of commands=
- <b>farm_init.run</b>: Initialize a farm. See [Quick start](#Quick_start.md) above.
- <b>submit.run</b>: Submit the farm to the scheduler. See [submit.run](#submit.run.md) below.
- <b>resubmit.run</b>: Resubmit all computations which failed or never ran as a new farm. See [Resubmitting failed cases](#Resubmitting_failed_cases.md).
...
for which please see [Large number of cases (META mode)](#Large_number_of_cases_(META_mode).md) below.
...
If the optional switch <code>-auto</code> is present, the farm will resubmit itself automatically at the end, more than once if necessary, until all the cases from table.dat have been processed. This feature is described at [Resubmitting failed cases automatically](META:_Advanced_features_and_troubleshooting#Resubmitting_failed_cases_automatically.md).

If a file named <code>final.sh</code> is present in the farm directory, <code>submit.run</code> will treat it as a job script for a post-processing job and it will be launched automatically once all the cases from table.dat have been successfully processed. See [Running a post-processing job automatically](META:_Advanced_features_and_troubleshooting#Running_a_post-processing_job_automatically.md) for more details.
...
```bash
...
1. ++++++++++++++++++++++  This part can be customized:  ++++++++++++++++++++++++
1. Here:
1. $ID contains the case id from the original table (can be used to provide a unique seed to the code etc)
1. $COMM is the line corresponding to the case $ID in the original table, without the ID field
1. $METAJOB_ID is the jobid for the current metajob (convenient for creating per-job files)
...
1. Executing the command (a line from table.dat)
1. It's allowed to use more than one shell command (separated by semicolons) on a single line
eval "$COMM"

1. Exit status of the code:
STATUS=$?
...
...
1. ++++++++++++++++++++++  This part can be customized:  ++++++++++++++++++++++++
1. Here we use $ID (case number) as a unique seed for Monte-Carlo type serial farming:
...
```bash
#!/bin/bash
1. Here you should provide the sbatch arguments to be used in all jobs in this serial farm
1. It has to contain the runtime switch (either -t or --time):
#SBATCH -t 0-00:10
#SBATCH --mem=4G
...
On Nibi you can copy it to your <code>~/bin</code> directory:
...
- analyzes all those <code>status.*</code> files (see [Output files](#Output_files.md) above);
...
On Alliance general-purpose clusters, a good choice would be 12h or 24h due to [scheduling policies](Job_scheduling_policies#Time_limits.md). 
...
The maximum number of jobs which can be submitted on Nibi and Rorqual is 1000, so you won't be able to run the whole collection with a single command.  
...
If this seems rather tedious, consider using an advanced feature of the META package for such large farms: [Resubmitting failed cases automatically](META:_Advanced_features_and_troubleshooting#Resubmitting_failed_cases_automatically.md). This will fully automate the farm resubmission steps.
...
See [META-Farm: Advanced features and troubleshooting](META-Farm:_Advanced_features_and_troubleshooting.md) for more detailed discussion of some features, and for troubleshooting suggestions.

If you need more help, contact [technical support](technical_support.md), mentioning the name of the package (META), and the name of the staff member who wrote the software (Sergey Mashchenko).
