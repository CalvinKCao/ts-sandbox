...
If you are porting a PyTorch program to one of our clusters, you should follow [our tutorial on the subject](Tutoriel_Apprentissage_machine.md).
...
PyTorch developers also offer [LibTorch](#LibTorch.md), which allows one to implement extensions to PyTorch using C++, and to implement pure C++ machine learning applications. Models written in Python using PyTorch can be converted and used in pure C++ through [TorchScript](https://pytorch.org/tutorials/advanced/cpp_export.html).
...
:1. Load a Python [module](Utiliser_des_modules/en#Sub-command_load.md), thus <code>module load python</code>
:2. Create and start a [virtual environment](Python#Creating_and_using_a_virtual_environment.md).
:3. Install PyTorch in the virtual environment with <code>pip install</code>. 

#### GPU and CPU
:```bash
prompt=(venv) [name@server ~]|pip install --no-index torch
```
...
```bash
prompt=(venv) [name@server ~]|pip install --no-index torch torchvision torchtext torchaudio
```
...
See [Ratios in bundles](Allocations_and_compute_scheduling#Ratios_in_bundles.md) for more information on the appropriate number of CPUs and Memory per GPU for each cluster.
...
y = torch.rand(5, 3)
print(y)
1. let us run the following only if CUDA is available
...
1. You may notice a significant slowdown when running the exact same GPU-enabled code with <code>torch < 1.12.0</code> and <code>torch >= 1.12.0</code>.
1. You may get different results when running the exact same GPU-enabled code with <code>torch < 1.12.0</code> and <code>torch >= 1.12.0</code>.
...
1. A much higher memory bandwidth than CPUs, which allows GPUs to efficiently use their massive number of cores to process much larger amounts of data per compute cycle.
...
1. Increase your <code>batch_size</code> to as much as you can fit in the GPU's memory to optimize your compute performance.
1. Use a <code>DataLoader</code> with as many workers as you have <code>cpus-per-task</code> to streamline feeding data to the GPU.

Of course, <code>batch_size</code> is also an important parameter with respect to a model's performance on a given task (accuracy, error, etc.) and different schools of thought have different views on the impact of using large batches. This page will not go into this subject, but if you have reason to believe that a small (relative to space in GPU memory) batch size is best for your application, skip to [Single GPU Data Parallelism](PyTorch#Single_GPU_Data_Parallelism.md) to see how to maximize GPU utilization with small inputs.
...
1. The $((SLURM_NTASKS_PER_NODE * SLURM_JOB_NUM_NODES)) variable tells the script how many processes are available for this execution. “srun” executes the script <tasks-per-node * nodes> times
...
1. PyTorch Lightning will query the environment to figure out if it is running inside a SLURM batch job
1. If it is, it expects the user to have requested one task per GPU.
1. If you do not ask for 1 task per GPU, and you do not run your script with "srun", your job will fail!
...
In cases where a model is fairly small, such that it does not take up a large portion of GPU memory and it cannot use a reasonable amount of its compute capacity, it is <b>not advisable to use a GPU</b>. Use [one or more CPUs](PyTorch#PyTorch_with_Multiple_CPUs.md) instead. However, in a scenario where you have such a model, but have a very large dataset and wish to perform training with a small batch size, taking advantage of Data parallelism on a GPU becomes a viable option. 
...
This code snippet will also load a checkpoint from <code>./ckpt</code>, if there is one, and continue training from that point. For more information, please refer to the [official PyTorch Lightning documentation](https://pytorch-lightning.readthedocs.io/en/stable/api/pytorch_lightning.callbacks.model_checkpoint.html).
...
```bash
prompt=$
ldd $VIRTUAL_ENV/lib/python3.11/site-packages/torch/lib/libtorch_cuda.so {{!
...
To test an application with CUDA, request an [interactive job](Running_jobs#Interactive_jobs.md) with a [GPU](Using_GPUs_with_Slurm.md).

= rTorch =

In order to install <tt>rTorch</tt>, from a login node:
...
2. Create your installation directory following [installing R packages](R#Installing_for_one_or_many_R_versions.md):
```bash
...
```bash
R -e 'install.packages("torch", repos{{=
...
```bash
srun --mem{{=
```3500M --gpus{{=}}h100_1g.10gb:1 -- R -e 'torch::torch_tensor(1)$cuda()'
|result=
