...
|-
|[Automation node](Automation_in_the_context_of_multifactor_authentication.md) : robot.tamia.ecpia.ca
|-
...
- Large and adjustable per group [quota](Storage_and_file_management#Filesystem_quotas_and_policies.md). 
...
[<tt>StdEnv/2023</tt>](Standard_software_environments/fr.md) is the standard environment on tamIA.

### GPU jobs
Jobs are assigned on whole nodes with one of the following options:

For jobs on a node with an H100 GPU: <code>--gpus=h100:4</code>

For jobs on a node with an H200 GPU: <code>--gpus=h200:8</code>

For jobs using several GPUs, options are <code>--gpus-per-nodes=h100:4</code> or <code>--gpus-per-nodes=h200:8</code>.
