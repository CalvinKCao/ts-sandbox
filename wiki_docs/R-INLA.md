...
The installation of the R-INLA package is a bit more complicated than most [R](R.md) packages, as it downloads
other pre-compiled executables that need to be made compatible with our [Standard software environment](Standard_software_environments.md).
...
**File: install_INLA_StdEnv2023.sh**
```bash
#!/bin/bash
...
**File: install_INLA_StdEnv2020.sh**
```bash
#!/bin/bash
...
- (4) Patch the pre-compiled executables so that they are compatible with our [Standard software environment](Standard_software_environments.md)