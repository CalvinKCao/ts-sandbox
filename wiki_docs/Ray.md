...
:```bash
prompt=(venv) [name@server ~]|pip install --no-index ray
```
...
1. 2. & 3. Create a virtualenv and install Ray on all nodes
1. Then, activate that virtualenv in this shell.
srun -N $SLURM_NNODES -n $SLURM_NNODES config_env.sh
source $SLURM_TMPDIR/ENV/bin/activate


1. Define Ray head node address (this node) & port (any free port will do)
...
1. Craft --multi-prog configuration for Ray
...
1. 1. Some older Ray versions malfunctioned if the worker nodes started before
1. the head node. In that case, please modify the worker-node launch configuration
1. above to add a 10-second delay, using the following alternative lines:
1. 1. cat <<EOF >"$ray_multiprog_config"
1. 0   ray start --block --head --node-ip-address=$HEAD_NODE --port=$RAY_PORT
1. *   sh -c 'sleep 10; exec "\$@"' sh ray start --block --address=${HEAD_NODE}:${RAY_PORT}
1. EOF
1. 1. Launch head and workers on any/all nodes allocated to the job.
...
1. Run your own script here
python test_ray.py "$@"
sleep 10


1. Shut down Ray worker nodes after the Python script exits.
1. Await its exit.
kill $ray_cluster_pid
wait


1. Stage out your results, if any
1. cp $SLURM_TMPDIR/results.json ~/scratch/
```
...
1. Check that Ray sees two nodes and their status is 'Alive'
print("Nodes in the Ray cluster:")
print(ray.nodes())

1. Check that Ray sees 12 CPUs and 2 GPUs over 2 Nodes
