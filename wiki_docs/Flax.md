...
:```bash
prompt=(venv) [name@server ~]|pip install --no-index flax
```
...
1. JIT Compile an entire training step: Forward and Backward in a single function call:
@jax.jit
...
1. "jax.pmap" parallelizes inputs, function evaluation and outputs over a given axis. 
1. This axis is first dimension of the inputs by default - it is the number of GPUs in this case.
1. jax.map also JIT compiles the function, just like @jax.jit in the single GPU case. 
...
**File: flax-example-distributed.sh**
```bash
#!/bin/bash
#SBATCH --nodes 2
#SBATCH --tasks-per-node=1
...
**File: flax-example-distributed.py**
```python
from flax import linen as nn
...
1. "jax.pmap" parallelizes inputs, function evaluation and outputs over a given axis. 
1. This axis is the first dimension of the inputs by default - it is the number of local GPUs in this case.
1. But because we called jax.distributed.initialize, computations will also be parallelized over nodes!
1. jax.map also JIT compiles the function, just like @jax.jit in the single GPU case.
