...
|content=VS Code is notable for misbehaving on the login nodes. Whenever possible **strictly** use it locally or see below how to configure it.
}}
...
- Use <tt>nano</tt> or <tt>vim</tt> to edit files directly on the systems.
...
1. Access one of the [options to launch JupyterLab](JupyterLab#Launching_JupyterLab.md).
1. Select minimal resources and start an interactive JupyterLab job.
1. On the Launcher tab, click on the VS Code launcher button.
...
## Connecting
1. Open the Command Palette in VS Code: Press <tt>Ctrl+Shift+P</tt> (Windows/Linux) or <tt>Cmd+Shift+P</tt> (macOS).
1. Type <tt>remote</tt> and then select <tt>Connect to Host...</tt>
1. Choose the host (remote system) and confirm.
...
1. Open the Command Palette in VS Code: Press <tt>Ctrl+Shift+P</tt> (Windows/Linux) or <tt>Cmd+Shift+P</tt> (macOS).
1. Type <tt>remote</tt> and then select <tt>Remote-SSH: Kill VS Code Server on Host...</tt>
1. Choose the host (remote system) and confirm.
1. Open the File menu, and select <tt>Close Remote Connection</tt>.
...
1. In an external terminal, connected to the system via an ssh connection, start a new <b>[interactive job](Running_jobs#Interactive_jobs.md)</b> (with <code>salloc</code>) with at least 2000M of memory.
## Note the allocated compute node name.
## If you need to work with <code>SLURM_*</code> environment variables in VS Code, save them all in a *source* file: ```bash
env {{!
``` grep SLURM_ {{!}} sed -e 's/^\(.*\){{=}}\(.*\)$/export \1{{=}}"\2"/g' > slurm_var.sh}}
1. In VS Code, start a new remote session with the name of the allocated compute node.
...
1. If you need to work with <code>SLURM_*</code> environment variables, navigate to the working directory in a VS Code terminal and *source* the <code>slurm_var.sh</code> file. ```bash
source slurm_var.sh
```
