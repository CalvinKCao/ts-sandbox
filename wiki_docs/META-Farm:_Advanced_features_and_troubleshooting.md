...
E.g. if the largest number of jobs one can submit on the cluster is 999 and you intend to use <code>-auto</code>, set <code>NJOBS_MAX</code> to 998. To find out the maximum number of submitted jobs limit (MaxSubmit) associated with your account on a specific cluster, run the following command:
...
This feature uses the same script, <code>resubmit_script.sh</code>, described for [<code>-auto</code>](#Resubmitting_failed_cases_automatically.md) above.
...
= WHOLE_NODE mode =
Starting from the version 1.0.3, meta-farm supports packaging individual serial farming jobs into whole node jobs. This made it possible to use the package on Trillium. This mode is off by default. To enable it, edit the file config.h inside your farm directory. Specifically, you need to set <code>WHOLE_NODE=1</code>, and set the variable <code>NWHOLE</code> to the number of CPU cores per node (192 for Trillium). 
...
Then proceed as shown in the META [Quick start](META:_A_package_for_job_farming#Quick_start.md) from the <code>farm_init.run</code> step.
...
For [multi-threaded](Running_jobs#Threaded_or_OpenMP_job.md) applications (such as those that use [OpenMP](OpenMP.md), for example), 
...
For applications that use [MPI](MPI.md), 
add the following lines to <code>job_script.sh</code>:
...
See [Advanced MPI scheduling](Advanced_MPI_scheduling.md) for information about more-complicated MPI scenarios.
...
For applications which use GPUs, modify <code>job_script.sh</code> following the guidance at [Using GPUs with Slurm](Using_GPUs_with_Slurm.md): 
...
1. $ID contains the case id from the original table
1. $COMM is the line corresponding to the case $ID in the original table, without the ID field
mkdir RUN$ID
cd RUN$ID

1. Converting $COMM to an array:
COMM=( $COMM )
1. Number of columns in COMM:
Ncol=${#COMM[@]}
1. Now one can access the columns individually, as ${COMM[i]} , where i=0...$Ncol-1
1. A range of columns can be accessed as ${COMM[@]:i:n} , where i is the first column
1. to display, and n is the number of columns to display
1. Use the ${COMM[@]:i} syntax to display all the columns starting from the i-th column
1. (use for codes with a variable number of command line arguments).

1. Call the user code here.
...
...
- The script measures the run-time of each case, and adds the value as one line in a scratch file <code>times</code> created in directory <code>/home/$USER/tmp/$NODE.$PID/</code>. (See [Output files](#Output_files.md).) This is done by all running meta-jobs.
...
See the discussion in section [job_script.sh](#job_script.sh.md) above.
...
*Parent page:* [META: A package for job farming](META:_A_package_for_job_farming.md)