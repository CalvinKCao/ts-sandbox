...
The <code>huggingface_hub</code> Python package contains a command line interface (CLI) which can be used to download models. For example, to download the model <tt>Zephyr-7b-beta</tt>, first install <code>huggingface_hub</code> in a [virtual environment](Virtualenv.md), then **on a login node** run:
