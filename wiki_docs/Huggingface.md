...
:1. Load a Python [module](Utiliser_des_modules/en#Sub-command_load.md), thus <tt>module load python</tt>
...
:```bash
prompt=(venv) [name@server ~]|pip install --no-index transformers
```
...
Pre-trained models are usually made up of fairly large binary files. The Hugging Face makes these files available for download via [Git Large File Storage](https://git-lfs.com/). To download a model, load the <tt>git-lfs</tt> module and clone your chosen model repository from the model hub:
...
Another frequently used way of loading a pre-trained model is via a  [pipeline](https://huggingface.co/docs/transformers/main_classes/pipelines). **On a login node** you can simply pass a model name or a type of task as an argument to <tt>pipeline</tt>. This will download and store the model at the default cache location:
...
:```bash
prompt=(venv) [name@server ~]|module load gcc/9.3.0 arrow/11.0.0
```
:```bash
prompt=(venv) [name@server ~]|pip install --no-index datasets
```
...
:```bash
prompt=(venv) [name@server ~]|module load gcc arrow
```
:```bash
prompt=(venv) [name@server ~]|pip install --no-index evaluate
```
...
:```bash
prompt=(venv) [name@server ~]|pip install --no-index accelerate
```

## Multi-GPU & multi-node jobs with Accelerate

In the example that follows, we use <tt>accelerate</tt> to reproduce our [PyTorch tutorial](PyTorch#PyTorch_with_Multiple_GPUs.md) on how to train a model with multiple GPUs distributed over multiple nodes. Notable differences are: 
