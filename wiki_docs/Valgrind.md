...
To get useful information from [Valgrind](http://valgrind.org/), you first need to compile your code with debugging information enabled. With the most compilers, you do so by adding a "<tt>-g</tt>" option during compilation.  
...
```bash
valgrind --tool{{=
```memcheck --leak-check{{=}}yes --show-reachable{{=}}yes ./your_program}}
