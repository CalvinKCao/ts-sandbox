...
1. From the <i>File Manager</i> screen click on the <i>Collections</i> icon on the left.
1. Click on <i>Get Globus Connect Personal</i> in the top right of the screen.
1. Click on the download link for your operating system (click on <i>Show me other supported operating systems</i> if downloading for another computer).
1. Install Globus Connect Personal.
1. You should now be able to access the endpoint through Globus. The full endpoint name is [your username]#[name you give setup]; for example, smith#WorkPC.
...
| In: <ul><li><code>/home</code>, yes (except on Rorqual).</li><li><code>/scratch</code>, no (except on Narval).</li><li><code>/project</code>, on demand (see below).</li></ul>
|}

On [general-purpose clusters](National_systems#Compute_clusters.md), Globus sharing is enabled for the <code>/home</code> directory, except on the Rorqual cluster. If you would like to test a Globus share you can create one in your <code>/home</code> directory.
...
[thumbnail|Open the <i>Share</i> option (click on image to enlarge)](File:Globus_SharedEndpoint1-1024x607.png.md)
Select a folder that you wish to share, then click the <i>Share</i> button to the right of the folder list.
<br clear=all>

[thumbnail|Click on <i>Add a Guest Collection</i> (click on image to enlarge)](File:Globus_SharedEndpoint2.png.md)
Click on the <i>Add Guest Collection</i> button in the top right corner of the screen. 
<br clear=all>

[thumbnail|Managing a shared collection (click on image to enlarge)](File:Globus_SharedEndpoint3-1024x430.png.md)
...
### Managing access
[thumbnail|Managing shared collection permissions (click on image to enlarge)](File:Globus_ManagingAccess-1024x745-changed.png.md)
...
[thumbnail|Removing a shared collection (click on image to enlarge)](File:Globus_RemovingSharedEndpoint-1024x322.png.md)
...
[thumbnail|Creating a Globus group (click on image to enlarge)](File:Globus_CreatingNewGroup-1024x717.png.md)
...
1. Create a virtual environment to install the Globus CLI into (see [creating and using a virtual environment](Python#Creating_and_using_a_virtual_environment.md)).```'console
$ virtualenv $HOME/.globus-cli-virtualenv
```
1. Activate the virtual environment. ```'console
$ source $HOME/.globus-cli-virtualenv/bin/activate
```
1. Install Globus CLI into the virtual environment (see [installing modules](Python#Installing_modules.md)).```'console
$ pip install globus-cli
```
1. Then deactivate the virtual environment.```'console'
$ deactivate
```
1. To avoid having to load that virtual environment every time before using Globus, you can add it to your path. ```'console
...
To access the Arbutus object storage, generate the storage <b>access ID</b> and <b>secret key</b> with the [OpenStack command line client](OpenStack_command_line_clients.md).<br>
...
3. Log into the [Globus portal](Globus#Using_Globus.md) at <https://www.globus.org/>(https://www.globus.org/).<br>
