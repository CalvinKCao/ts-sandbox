...
[List of TCP and UDP port numbers](https://en.wikipedia.org/wiki/List_of_TCP_and_UDP_port_numbers) for more.
}}
...
[Jupyter notebook](Jupyter#Starting_Jupyter_Notebook.md), for example, you are given a URL with a token:
```
...
*Session 1 should be a connection to a cluster. Start your job there following the instructions for your application, such as [Jupyter Notebook](Jupyter#Starting_Jupyter_Notebook.md). You should be given a URL that includes a host name and a port, such as <code>fc3281.int.fir.alliancecan.ca:8888</code> for example.
...
Again using an example from [Jupyter Notebook](Jupyter#Starting_Jupyter_Notebook.md), this would be the URL to paste into a browser:
```
