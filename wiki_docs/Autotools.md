...
In other cases you must supply the <tt>--prefix</tt> option to <tt>./configure</tt> instead of to <tt>make</tt>; see the documentation for the particular software you are trying to install. You may also wish to [create a module](Créer_un_module.md) to show the system the paths to your newly installed software.
...
```bash
./configure --enable-mpi --with-mpi-dir{{=
```$MPIDIR}}
