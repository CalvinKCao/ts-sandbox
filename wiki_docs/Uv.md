...
{{Warning
|title=Not supported
|content=uv is currently **not** supported on our clusters.
}}
...
To install packages on our clusters, use <code>pip</code>; see [Python](Python.md).

Make sure to use at least <code>pip>=25.0</code>.
```bash
prompt=(ENV) [name@server ~]|pip install --no-index --upgrade pip
```
...
uv cache clean
```
Then, to avoid using the cache, use <code>uv --no-cache</code>.