...
See section [Finding which of your jobs should use an instance](Multi-Instance_GPU#Finding_which_of_your_jobs_should_use_an_instance.md) for more details.
...
GPU jobs requiring many CPU cores may also require a full GPU instead of an instance. The maximum number of CPU cores per instance depends on [the number of cores per full GPU](Allocations_and_compute_scheduling#Ratios_in_bundles.md) and on the configured [MIG profiles](https://docs.nvidia.com/datacenter/tesla/mig-user-guide/index.html#a100-profiles). Both vary between clusters and between GPU nodes in a cluster.
...
- [Narval, with NVIDIA A100-40gb GPUs](Narval/en#GPU_instances.md)
- [Rorqual, with NVIDIA H100-80gb GPUs](Rorqual/en#GPU_instances.md)
- Nibi:
...
For example, a <code>3g.20gb</code> instance has 20 GB of GPU memory and offers 3/8<sup>th</sup> of the computing performance of a full GPU. Using less powerful profiles will have a lower impact on your allocation and priority.
...
```bash
sinfo -o "%G"{{!
```grep gpu{{!}}sed 's/gpu://g'{{!}}sed 's/),/\n/g'{{!}}cut -d: -f1{{!}}sort{{!}}uniq}}

The recommended maximum number of CPU cores and amount of system memory per instance are listed in the [table of ratios in bundles](Allocations_and_compute_scheduling#Ratios_in_bundles.md).
...
[400px|frame|left|Example GPU Power usage of a job on a full A100 GPU](File:ExampleGPUPower.png.md) 
...
[400px|frame|left|Example GPU Utilization of a job on a full A100 GPU](File:ExampleGPUUtilisation.png.md)
<br clear=all> <!-- This is to prevent the next section from filling to the right of the image. -->
...
[400px|frame|left|Example GPU memory usage of a job on a full A100 GPU](File:ExampleGPUMemory.png.md)
<br clear=all> <!-- This is to prevent the next section from filling to the right of the image. -->
...
Another way to monitor the usage of a running job is by [attaching to the node](Running_jobs#Attaching_to_a_running_job.md) where the job is currently running and then by using <code>nvidia-smi</code> to read the GPU metrics in real time.
...
No.  While this is possible in principle, we don't support this.  If you want to run multiple independent tasks on a GPU, you should use [MPS](Hyper-Q_/_MPS.md) rather than MIG.