...
salloc --time{{=
```0:30:0 --ntasks{{=}}1 --mem-per-cpu{{=}}3600 --account{{=}}def-someprof
...
module load gcc/12.3 chapel-multicore/2.4.0
salloc --time{{=
```0:30:0 --ntasks{{=}}1 --cpus-per-task{{=}}3 --mem-per-cpu{{=}}3600 --account{{=}}def-someprof
|chpl test.chpl -o test
|./test
}}
For production jobs, please write a [job submission script](Running_jobs.md) and submit it with <code>sbatch</code>.
...
salloc --time{{=
```0:30:0 --nodes{{=}}4 --cpus-per-task{{=}}3 --mem-per-cpu{{=}}3500 --account{{=}}def-someprof
}}
...
For production jobs, please write a [Slurm submission script](Running_jobs.md) and submit your job with <code>sbatch</code> instead.
...
salloc --time{{=
```0:30:0 --mem-per-cpu{{=}}3500 --gpus-per-node{{=}}1 --account{{=}}def-someprof
}}
