...
You can still use wandb by enabling the [<code>offline</code>](https://docs.wandb.ai/library/cli#wandb-offline) mode. In this mode, wandb will write all metrics, logs and artifacts to the local disk and will not attempt to sync anything to the Weights&Biases service on the internet. After your jobs finish running, you can sync their wandb content to the online service by running the command [<code>wandb sync</code>](https://docs.wandb.ai/ref/cli#wandb-sync) on the login node.
