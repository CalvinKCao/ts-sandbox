...
1. **Analysis**: Visualize the data produced in the first step.
Both steps can be accomplished in either command-line mode or graphical mode. 

## Environment modules
Before you start profiling with PGPROF, the appropriate [module](Utiliser_des_modules.md) needs to be loaded.  
...
[thumbnail|300px|Starting a new PGPROF session (click for a larger image)|left](File:pgprof-start-session.png.md)
...
** Since the Pgrof's GUI is based on Java, it should be executed on the compute node in the interactive session rather than on the login node, as the latter does not have enough memory (see [Java](Java#Pitfalls.md) for more details). An interactive session can be started with <code>salloc --x11 ...</code> to enable X11 forwarding (see [Interactive jobs](Running_jobs#Interactive_jobs.md) for more details). 
...
[thumbnail|300px|Visualizing performance data (click for a larger image)|left](File:pgprof2.png.md)
