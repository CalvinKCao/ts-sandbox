...
man command
```
<code>man</code> uses <code>less</code> (see the section [Viewing and editing files](#Viewing_and_editing_files.md)), and you must press <code>q</code> to exit this program.
...
- Prevent group members and all others to read or modify the file <code>secret.txt</code>: ```bash
chmod go-rwx secret.txt
```
- Allow everybody to read the file <code>public.txt</code>: ```bash
chmod a+r public.txt
```
- Make the file <code>script.sh</code> executable: ```bash
chmod a+x script.sh
```
- Allow group members to read and write in the directory <code>shared</code>: ```bash
chmod g+rwx shared
```
- Prevent other users from reading or modifying your home directory: ```bash
chmod go-rw ~
```
