...
1. Click on the start button and begin typing <i>command prompt</i>.
1. Right-click on the Command Prompt application and select <i>Run as administrator</i>. Accept any security prompt that appears.
1. In the Command Prompt window, type the following command, and wait for it to complete: ```bash
prompt=C:\>|wsl --install
```
...
1. At the prompt <i>Enter new UNIX username</i>, enter your desired username and press enter.
1. At the prompt <i>Enter new UNIX password</i>, enter your desired password and press enter. You will not see characters as you type them; this is normal.
...
1. By automatically mounting (attaching) your Windows drives within the Linux folder structure at <i>/mnt/</i>.
1. By adding a Linux entry in the Windows Explorer sidebar that provides direct access to files stored within Linux.
...
1. Note the drive letter (e.g., <i>C:\</i>).
1. Replace the drive letter with <i>/mnt/{letter}/</i>.
1. Transform all of the backslashes to forward slashes.
...
1. Use Windows Explorer’s sidebar to find the Linux entry (usually near the bottom) and expand it.
1. Select the Linux environment that contains the file (<i>Ubuntu</i> by default).
1. Navigate through the same folder structure from step 1 to find the file/folder.
...
1. Run ```bash
prompt=[name@ubuntu-wsl ~]$|explorer.exe .
...
```bash
prompt=[name@ubuntu-wsl ~]$|sudo apt install filezilla
```
