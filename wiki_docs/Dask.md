...
:1. Load a Python [module](Utiliser_des_modules/en#Sub-command_load.md), thus <tt>module load python/3.11</tt>
...
:```bash
prompt=(venv) [name@server ~]|pip install --no-index dask distributed
```
...
if ["$SLURM_PROCID" -eq "0"]("$SLURM_PROCID"_-eq_"0".md); then
