...
Provides reusable methods for visualizing predictions vs ground truth
"""
...
class TimeSeriesVisualizer:
    """
    A comprehensive visualization toolkit for time series forecasting
    ...
    def plot_time_series_comparison(self,
        ...
        """
        Plot comparison between real and predicted time series
        
        Args:
            real_data: Real time series data (N, seq_len, channels) or (N, channels, seq_len)
            generated_data: Predicted time series data (same shape as real_data)
        ...
        for i, idx in enumerate(indices):
            for j in range(num_features):
                ...
                # If input data provided, plot it first (lookback window)
                if input_data is not None:
                    input_len = input_data.shape[1]
                    x_input = np.arange(0, input_len)
                    ax.plot(x_input, input_data[idx, :, j], 
                           label='Input (Lookback)', alpha=0.6, linewidth=2, color='gray')
                    
                    # Plot predictions starting after input
                    x_pred = np.arange(input_len, input_len + seq_len)
                    ax.plot(x_pred, real_data[idx, :, j], 
                           label='Ground Truth', alpha=0.7, linewidth=2, color='blue')
                    ax.plot(x_pred, generated_data[idx, :, j], 
                           label='Prediction', alpha=0.7, linewidth=2, linestyle='--', color='orange')
                    
                    # Add vertical line to separate input from prediction
                    ax.axvline(x=input_len, color='red', linestyle=':', alpha=0.5, linewidth=1.5)
                else:
                    # Plot without input data (original behavior)
                    ax.plot(real_data[idx, :, j], label='Ground Truth', alpha=0.7, linewidth=2)
                    ax.plot(generated_data[idx, :, j], label='Prediction', alpha=0.7, linewidth=2, linestyle='--')
        ...
        plt.suptitle(f'{title_prefix}Ground Truth vs Predictions', fontsize=16, fontweight='bold', y=1.002)
        plt.tight_layout()
    ...
    def plot_random_samples(self,
        ...
        """
        Plot random samples from real and predicted data
        
        Args:
            real_data: Real time series data
            generated_data: Predicted time series data
    ...
    def plot_feature_distributions(self,
        ...
        for j in range(num_features):
            ...
            # Plot histograms
            ax.hist(real_feature, bins=50, alpha=0.5, label='Ground Truth', density=True, color='blue')
            ax.hist(gen_feature, bins=50, alpha=0.5, label='Prediction', density=True, color='orange')
    ...
    def plot_pca_embedding(self,
        ...
        """
        Plot PCA embedding of real vs predicted data
        ...
        if n_components == 2:
            ...
            ax.scatter(real_embedded[:, 0], real_embedded[:, 1], 
                      alpha=0.5, label='Ground Truth', s=20)
            ax.scatter(gen_embedded[:, 0], gen_embedded[:, 1], 
                      alpha=0.5, label='Prediction', s=20)
        ...
        plt.title('PCA Embedding: Ground Truth vs Predictions', fontsize=16, fontweight='bold')
        plt.tight_layout()
    ...
    def plot_tsne_embedding(self,
        ...
        """
        Plot t-SNE embedding of real vs predicted data
        ...
        fig, ax = plt.subplots(1, 1, figsize=(10, 8))
        ax.scatter(real_embedded[:, 0], real_embedded[:, 1], 
                  alpha=0.5, label='Ground Truth', s=20, color='blue')
        ax.scatter(gen_embedded[:, 0], gen_embedded[:, 1], 
                  alpha=0.5, label='Prediction', s=20, color='orange')
        ax.set_xlabel('t-SNE Dimension 1', fontsize=12)
        ax.set_ylabel('t-SNE Dimension 2', fontsize=12)
        ax.legend(fontsize=12)
        ax.grid(True, alpha=0.3)
        
        plt.title('t-SNE Embedding: Ground Truth vs Predictions', fontsize=16, fontweight='bold')
        plt.tight_layout()
    ...
    def plot_statistical_summary(self,
        ...
        """
        Plot statistical summary comparing real and predicted data
        ...
        for idx, (stat_name, (real_stat, gen_stat)) in enumerate(stats.items()):
            ax = axes[idx]
            ax.bar(x - width/2, real_stat, width, label='Ground Truth', alpha=0.8)
            ax.bar(x + width/2, gen_stat, width, label='Prediction', alpha=0.8)
        ...
        plt.suptitle('Statistical Summary: Ground Truth vs Predictions', fontsize=16, fontweight='bold')
        plt.tight_layout()
