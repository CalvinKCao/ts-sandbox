...
#   - binary: no Gaussian lattice jitter (--no-binary-debias-quantization default)
#   - mmpd: snap each timestep to nearest global ordinal ladder rung (binary bins)
...
exec "$REPO_ROOT/slurm_discriminator_texture_staged_vs_mmpd.sh" \
    ...
    --test-stride 1 \
    --test-fraction 0.125 \
    --disc-run disc-lb336-hz720-ordinal-four-mmpd-binned \
    --raw-run disc-lb336-hz720-ordinal-four-raw \
    --report reports/disc_texture_lb336_hz720_ordinal_four_mmpd_binned.md \
