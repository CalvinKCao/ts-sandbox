#!/bin/bash
# Fair univariate binary-vs-MMPD discriminator campaign.
#
# One model per dataset; each example is a single-variate L patch;
# label = binary_staged (1) vs mmpd (0). Reuses trainval25 raw packs.
#
...
#   DISC_NATIVE_REPR_STRIDE=2 ./temp/submit_discriminator_binary_vs_mmpd_univariate_fair_killarney.sh \
#     --disc-run disc-lb336-hz720-ordinal-four-binary-vs-mmpd-univariate-stride2
#
set -euo pipefail
...
# ordinal snap + candidate-only + nonoverlap + no offset emb are hardcoded in the slurm worker
...
exec "$REPO_ROOT/slurm_discriminator_binary_vs_mmpd_univariate.sh" \
    ...
    --pack-fraction 0.25 \
    --force-train \
    --disc-run disc-lb336-hz720-ordinal-four-binary-vs-mmpd-univariate \
