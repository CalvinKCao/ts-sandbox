...
#   ./temp/submit_viz_g_ablation_finetune_maps_killarney.sh --n-windows 4 --g-values 1-10
#
# Outputs:
...
elif [[ -n "${SLURM_SUBMIT_DIR:-}" && -f "$SLURM_SUBMIT_DIR/temp/viz_g_ablation_finetune_maps.py" ]]; then
    REPO="$SLURM_SUBMIT_DIR"
else
    REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
fi
SCRIPT_DIR="$REPO/temp"
VIZ_PY="temp/viz_g_ablation_finetune_maps.py"
