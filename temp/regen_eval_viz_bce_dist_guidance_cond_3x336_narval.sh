#!/bin/bash
# Rebuild and log eval plots from the existing Jul 18 hz720 guidance-cond checkpoints on Narval A100 nodes.
set -euo pipefail
./submit_binary.sh --resume \
  ...
  --datasets ETTh1,traffic,exchange_rate --gpu a100 --time 0:30:00 "$@"
