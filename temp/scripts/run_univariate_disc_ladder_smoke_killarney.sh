...
# paper instance-normalized MMPD Decoder checkpoints, then train a tiny
# univariate discriminator in binary's shared dataset-z / 256-bin decode space.
set -euo pipefail
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
    ...
    python3 -u "$REPO/temp/scripts/check_mmpd_instance_campaign.py" "$MMPD_ROOT"
    [[ -d "$REPO/temp/MMPD" ]] || {
    ...
    for DATASET in "${DATASET_LIST[@]}"; do
        ...
        sbatch \
            ...
            "$REPO/temp/scripts/run_univariate_disc_ladder_smoke_killarney.sh"
    done
    exit 0
fi
...
COMMON_ARGS=(
    ...
    --no-mmpd-ordinal-norm --mmpd-instance-norm
...
python -u "$REPO/temp/scripts/inspect_univariate_disc_ladder.py" \
    "${COMMON_ARGS[@]}" \
    --output-dir "$DISC_DIR" \
    --force-raw-eval
...
python -u "$REPO/temp/scripts/plot_univariate_disc_ladder.py" \
    "${COMMON_ARGS[@]}" \
    --output-dir "$DISC_DIR" \
