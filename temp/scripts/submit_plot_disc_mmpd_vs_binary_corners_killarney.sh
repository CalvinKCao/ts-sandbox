...
#   ./temp/scripts/submit_plot_disc_mmpd_vs_binary_corners_killarney.sh
#   ./temp/scripts/submit_plot_disc_mmpd_vs_binary_corners_killarney.sh --dataset electricity --slice-len 8
#
set -euo pipefail
...
python -u "$REPO/temp/scripts/plot_disc_mmpd_vs_binary_confusion_corners.py" \
    --dataset "$DATASET" \
    --slice-len "$SLICE_LEN" \
    --disc-run "$DISC_RUN" \
    --raw-run "$RAW_RUN" \
