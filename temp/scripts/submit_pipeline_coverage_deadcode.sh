...
#   ./temp/scripts/submit_pipeline_coverage_deadcode.sh --dry-run
# =============================================================================
...
PY
    ...
    python3 "$REPO/temp/scripts/make_coverage_synth_dataset.py"
    ...
    if [[ "$DRY_RUN" -eq 1 ]]; then
        ...
        python3 "$REPO/temp/scripts/run_pipeline_coverage_deadcode.py" --dry-run \
...
python -u "$REPO/temp/scripts/make_coverage_synth_dataset.py"
...
python -u "$REPO/temp/scripts/run_pipeline_coverage_deadcode.py" \
    --run-name "$RUN_NAME" \
    --results-root "$REPO/results" \
    "${EXTRA_ARGS[@]}"
