...
Mirrors ``temp/viz_lull_etth2_c128_snap/`` for the recent non-hybrid canvas128
electricity leaf (train **4597054**). Fail-fast if an ordinal absolute ladder or
hybrid flat mask is selected.

Panels (under ``temp/viz_electricity_v2_c128_snap/``):

1. ``pre_post_snap/col2_v2/`` — GT / binary / MMPD pre-snap vs post-snap onto the
...
DATASET = "electricity"
SUBSET_ID = "electricity_4v_s1"
VARIATE = 2
# CSV columns after date are "0","1","2","3"; subset keeps first 4 → local idx 2 = col "2".
VARIATE_NAME = "2"
VARIATE_TAG = "col2_v2"
...
DEFAULT_CFG = "configs/binary_window_norm_patch_refine_canvas128_p64x6_electricity.yaml"
DEFAULT_OUT = REPO_ROOT / "temp" / "viz_electricity_v2_c128_snap"
# High |binary−MMPD| / MAE-delta pools on v=2 from the 1845 pack.
PREFERRED_POOLS = (1916, 1913, 1914, 1912)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--variate", type=int, default=VARIATE)
...
def _select_locals(
    *,
    ...
    """Prefer forced / known high-Δ pools, then high |binary−MMPD|, then MAE."""
    n = int(gt.shape[0])
...
def _title_prefix(*, variate: int, pool: int, local: int) -> str:
    return (
        f"{RUN_NAME}/{DATASET} {VARIATE_TAG} (col={VARIATE_NAME} idx={variate}) "
        f"pool={pool} local={local}"
    )


def _file_stem(*, variate: int, local: int, pool: int) -> str:
    return f"{RUN_NAME}_{DATASET}_{VARIATE_TAG}_local{local}_pool{pool}"
...
def _write_horizon96(
    *,
    ...
    for local in locals_:
        ...
        ax.set_title(
            f"{_title_prefix(variate=variate, pool=pool, local=local)} | "
        ...
        ax.grid(alpha=0.2)
        fig.tight_layout()
        path = out_dir / f"{_file_stem(variate=variate, local=local, pool=pool)}_H{gt.shape[-1]}.png"
...
def _write_snapproof(
    *,
    ...
    for local in locals_:
        ...
        path = out_dir / (
            f"{_file_stem(variate=variate, local=local, pool=pool)}_"
...
def _write_gt_bins(
    *,
    ...
    for local in locals_:
        ...
        t = np.arange(h)
        path = h96_dir / (
            f"{_file_stem(variate=variate, local=local, pool=pool)}_H{h}_gt_bins.png"
        )
        _plot_gt_bins(
            out_path=path,
            title=(
...
def _write_readme(out_dir: Path, meta: Mapping[str, Any]) -> None:
    ...
    lines = [
        f"# electricity {VARIATE_TAG} (CSV col `{VARIATE_NAME}`, subset idx {meta['variate']}) "
        "window_norm_grid canvas128 snap viz",
        "",
        "Generated by `temp/scripts/viz_electricity_v2_c128_snap.py`.",
        "",
        "Same panel batch as `temp/viz_lull_etth2_c128_snap/` for the electricity "
        "canvas128 leaf (non-hybrid window_norm).",
        "",
        "## Snap contract",
        "",
        ...
        "",
        f"- pack: `{meta['pack']}`",
        f"- ckpt: `{meta['ckpt']}` (train job **4597054**)",
        f"- config: `{meta['config']}`",
        f"- subset: `{meta['subset_id']}` (variate_indices={meta.get('variate_indices')})",
        ...
        "## Layout",
        "",
        f"- `pre_post_snap/{VARIATE_TAG}/` — H=96 pre vs post-snap; L=8/16 pre→snap→bin_center "
        "(+ optional zscore NOT-live); bin-index panels",
        ...
        "",
        f"## Panels (variate={meta['variate']} CSV col `{VARIATE_NAME}`)",
        "",
        f"- pools: {meta['pools']}",
    ...
    lines.extend(
        [
            ...
            "python temp/scripts/viz_electricity_v2_c128_snap.py --cpu --n-windows 4",
            "```",
            "",
        ]
    )
...
def main() -> None:
    ...
    state = _build_state(args.ckpt, DATASET, SUBSET_ID, args.config)
    if bool(state.use_ordinal_window_norm):
        raise RuntimeError(
    ...
    if flat_mask is not None and any(flat_mask):
        raise RuntimeError(
            ...
            "wrong leaf for this window_norm_grid foil"
        )
    if bool(getattr(state, "hybrid_flat_dataset_norm", False)):
        raise RuntimeError(
    ...
    print(
        f"[elec-v2-wn128] snap N={binary_pack['y_true'].shape[0]} "
    ...
    print(
        f"[elec-v2-wn128] locals={locals_} pools={pools} snap_mode={snap_mode}",
        flush=True,
    )

    args.output_dir.mkdir(parents=True, exist_ok=True)
    pre_dir = args.output_dir / "pre_post_snap" / VARIATE_TAG
    ...
    for local in locals_:
        ...
        title = f"{_title_prefix(variate=v, pool=pool, local=local)} | {WN128_LATTICE}"
        pre = {
            "GT": gt_pre[local, v],
    ...
    if not args.skip_gt_bins:
        print("[elec-v2-wn128] encoding GT coarse/fine on window-norm path…", flush=True)
    ...
    if locals_ and pools:
        ...
        starter = [
            ...
            _rel(
                ...
                / "L8_snapproof"
                / (
                    f"{_file_stem(variate=v, local=local0, pool=pool0)}_"
    ...
    variate_indices = list((run.metadata or {}).get("variate_indices") or [0, 1, 2, 3])
    meta = {
        ...
        "fallback_note": (
            "none — used 4597054 non-hybrid + 08-04-1845 pack "
            "(binary+MMPD index-aligned; 08-04-2341 wn128grid pack not local)"
        ),
        "mmpd_note": (
            "mmpd_electricity_val-test.npz shipped with 08-04-1845 disc leaf "
            "(exact index match to binary_window_norm_c128_electricity_val-test.npz)"
        ),
    }
