...
#   sbatch --exclude=kn010 ./temp/scripts/submit_rerun_ordinal_disc_confusions_h96.sh
#   sbatch --exclude=kn010 ./temp/scripts/submit_rerun_ordinal_disc_confusions_h96.sh --smoke-test
# =============================================================================

set -euo pipefail

REPO="${SLURM_SUBMIT_DIR:-}"
if [[ -z "$REPO" || ! -f "$REPO/temp/scripts/rerun_ordinal_disc_confusions_h96.py" ]]; then
...
PY_SCRIPT="temp/scripts/rerun_ordinal_disc_confusions_h96.py"
PY_ARGS=("$@")
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
    ...
    echo "Submitting ordinal disc confusion rerun from $REPO ..."
    echo "  python $PY_SCRIPT ${PY_ARGS[*]:-}"
    sbatch \
        --chdir="$REPO" \
        --exclude=kn010 \
        --job-name="disc-conf-h96" \
        ...
        --cpus-per-task="$CPUS" \
        --mem="$MEM" \
        --output="$REPO/results/logs/disc-conf-h96-%j.out" \
        --error="$REPO/results/logs/disc-conf-h96-%j.err" \
        --mail-type=END,FAIL \
        --mail-user=ccao87@uwo.ca \
        "$SCRIPT_DIR/submit_rerun_ordinal_disc_confusions_h96.sh" "${PY_ARGS[@]}"
    exit 0
fi
