...
def _eval_checkpoint_dir(state: PipelineState) -> str:
    """Return a separately configured checkpoint root for evaluation-only runs."""
...
def _source_stage_best_ckpt(state: PipelineState, stage: str) -> str:
    return os.path.join(
        _eval_checkpoint_dir(state), state.subset_id or state.dataset, stage, "best.pt"
    )
...
def _summarize_anchor_only_metrics(pack: Dict[str, np.ndarray]) -> Dict[str, float]:
    """Report deterministic metrics without treating repeated anchors as MC samples."""
...
def _load_stage_metadata(state: PipelineState, stage: str) -> Dict:
    meta = os.path.join(os.path.dirname(_source_stage_best_ckpt(state, stage)), "metadata.json")
...
def _stage_finetune_ckpt(state: PipelineState, stage: str) -> str:
    ...
    if value and os.path.exists(value):
        return value
    path = _source_stage_best_ckpt(state, stage)
...
class StagedEvalPhase(PipelinePhase):
    ...
    def _load_model(self, state: PipelineState, stage: str, itrans_guidance, n_iv: int, device: torch.device):
        ...
        state = stage_state(state, stage, honor_dataset_windows=True)
