...
#   ./temp/scripts/submit_viz_past_native_etth1_gt_pred_maps_killarney.sh
#   ./temp/scripts/submit_viz_past_native_etth1_gt_pred_maps_killarney.sh --g-labels 1.0,4.0 --n-windows 4
#
...
elif [[ -n "${SLURM_SUBMIT_DIR:-}" && -f "$SLURM_SUBMIT_DIR/temp/scripts/viz_past_native_etth1_gt_pred_maps.py" ]]; then
    REPO="$SLURM_SUBMIT_DIR"
else
    REPO="$(cd "$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")/../.." && pwd)"
fi
SCRIPT_DIR="$REPO/temp/scripts"
VIZ_PY="temp/scripts/viz_past_native_etth1_gt_pred_maps.py"
