...
#   ./temp/scripts/submit_viz_synth_pretrain_killarney.sh
#   ./temp/scripts/submit_viz_synth_pretrain_killarney.sh \
...
elif [[ -n "${SLURM_SUBMIT_DIR:-}" && -f "$SLURM_SUBMIT_DIR/temp/scripts/viz_synth_pretrain.py" ]]; then
    REPO="$SLURM_SUBMIT_DIR"
else
    REPO="$(cd "$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")/../.." && pwd)"
fi
SCRIPT_DIR="$REPO/temp/scripts"
VIZ_PY="temp/scripts/viz_synth_pretrain.py"
