...
#   ./temp/scripts/submit_viz_sequential_ordinal_exchange_prob_samples_killarney.sh
#   ./temp/scripts/submit_viz_sequential_ordinal_exchange_prob_samples_killarney.sh --n-windows 5 --n-prob-samples 10
#   ./temp/scripts/submit_viz_sequential_ordinal_exchange_prob_samples_killarney.sh --n-windows 1 --n-prob-samples 1  # smoke
#
# Outputs under:
...
elif [[ -n "${SLURM_SUBMIT_DIR:-}" && -f "$SLURM_SUBMIT_DIR/temp/scripts/visualize_sequential_ordinal_exchange_prob_samples.py" ]]; then
    REPO="$SLURM_SUBMIT_DIR"
else
    REPO="$(cd "$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")/../.." && pwd)"
fi
SCRIPT_DIR="$REPO/temp/scripts"
VIZ_PY="temp/scripts/visualize_sequential_ordinal_exchange_prob_samples.py"
