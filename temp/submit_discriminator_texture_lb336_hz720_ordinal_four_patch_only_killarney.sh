...
#   ./temp/submit_discriminator_texture_lb336_hz720_ordinal_four_patch_only_killarney.sh --smoke-test
#   ./temp/submit_discriminator_texture_lb336_hz720_ordinal_four_patch_only_killarney.sh
#
set -euo pipefail
...
export DISC_MMPD_ORDINAL_QUANTIZE=1
...
exec "$REPO_ROOT/slurm_discriminator_texture_staged_vs_mmpd.sh" \
    ...
    --test-stride 1 \
    --test-fraction 0.125 \
    --disc-run disc-lb336-hz720-ordinal-four-patch-only \
    --raw-run disc-lb336-hz720-ordinal-four-raw \
    --report reports/disc_texture_lb336_hz720_ordinal_four_patch_only.md \
