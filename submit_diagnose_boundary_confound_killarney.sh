...
# Identity schedule; metrics on full vs fill_only (drop K=lookback_overlap prefix).
#
# USAGE (Killarney login, from $SCRATCH/ts-sandbox):
#   ./submit_diagnose_boundary_confound_killarney.sh --datasets exchange_rate,electricity
#   for ds in exchange_rate electricity weather ETTh1 traffic; do
#     ./submit_diagnose_boundary_confound_killarney.sh --datasets "$ds"
#   done
#
set -euo pipefail
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
    ...
    JOB_NAME="noise-boundary-confound"
    for ((i = 0; i < ${#EXTRA[@]}; i++)); do
        if [[ "${EXTRA[$i]}" == "--datasets" && $((i + 1)) -lt ${#EXTRA[@]} ]]; then
            JOB_NAME="noise-bnd-${EXTRA[$((i + 1))]%%,*}"
            break
        fi
    done
    ...
    mkdir -p "$LOG_DIR"
    DATE_STR="$(date +%m-%d)"
    LOG_FILE="$LOG_DIR/${DATE_STR}-noise-boundary-confound-%j.log"
    ...
    exec sbatch \
        ...
        "$SCRIPT_DIR/submit_diagnose_boundary_confound_killarney.sh" \
        "${EXTRA[@]}"
fi
...
echo "Started: $(date)"
echo "Repo:   $REPO"
echo "datasets=$DATASETS n_samples=$N_SAMPLES ma_window=$MA_WINDOW"
echo "=========================================="
...
OUT_DIR="$REPO/reports/noise_boundary_confound_${SLURM_JOB_ID}"
PY_ARGS=(
    utils/diagnose_boundary_confound_noise_metrics.py
    --datasets "$DATASETS"
    --n-samples "$N_SAMPLES"
    --stage "$STAGE"
    --ma-window "$MA_WINDOW"
